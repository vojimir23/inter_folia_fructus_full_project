...
def process_items(raw_items: List[Dict[str, Any]], context: Dict[str, Any], manifestations_data_map: Dict[str, Any], works_data_map: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    item_to_manifestation_ids = context.get("item_to_manifestation_ids", {})
    ...
    for item in raw_items:
        ...
        parent_manifestation_ids = item_to_manifestation_ids.get(item_id, [])
        if not parent_manifestation_ids and (single_mid := item_to_manifestation.get(item_id)):
            parent_manifestation_ids = [single_mid]
        ...
        for p_mid in parent_manifestation_ids:
            if p_man_details := manifestations_data_map.get(p_mid):
                if manifestation_title == "Unknown Manifestation":
                    manifestation_title = p_man_details.get("manifestation_title", manifestation_title)
                
                for e_id in p_man_details.get("expression_ids", [p_man_details.get("expression_id")] if p_man_details.get("expression_id") else []):
                    if e_id and e_id not in expression_ids:
                        expression_ids.append(e_id)
                if expression_title == "Unknown Expression":
                    expression_title = p_man_details.get("expression_title", expression_title)
                    
                for w_id in p_man_details.get("work_ids", [p_man_details.get("work_id")] if p_man_details.get("work_id") else []):
                    if w_id and w_id not in work_ids:
                        work_ids.append(w_id)
                if work_title == "Unknown Work":
                    work_title = p_man_details.get("work_title", work_title)

                for a in p_man_details.get("authors", []):
                    if a not in authors_from_work:
                        authors_from_work.append(a)
                if not id2projects.get(item_id):
                    projects = p_man_details.get("projects", ["Unknown Project"])

                for cl in p_man_details.get("classifications", []):
                    if cl not in classifications: classifications.append(cl)
                for toe in p_man_details.get("type_of_expression", []):
                    if toe not in type_of_expression: type_of_expression.append(toe)
                for lang in p_man_details.get("language", []):
                    if lang not in language: language.append(lang)
                if not publication_place: publication_place = p_man_details.get("publication_place")
                if not publication_date: publication_date = p_man_details.get("publication_date")
                if not publication_date_range: publication_date_range = p_man_details.get("publication_date_range")
                if not publication_start_year: publication_start_year = p_man_details.get("publication_start_year")
                if not publication_end_year: publication_end_year = p_man_details.get("publication_end_year")

                classifications_normalized.update(p_man_details.get("classifications_normalized", set()))
                type_of_expression_normalized.update(p_man_details.get("type_of_expression_normalized", set()))
                language_normalized.update(p_man_details.get("language_normalized", set()))
                publication_place_normalized.update(p_man_details.get("publication_place_normalized", set()))
                work_title_normalized.update(p_man_details.get("work_title_normalized", set()))
                author_search_terms_normalized.update(p_man_details.get("author_search_terms_normalized", set()))
        ...
        item_data = {
            ...
            "manifestation_ids": parent_manifestation_ids,
