...
def process_expressions(raw_expressions: List[Dict[str, Any]], context: Dict[str, Any], works_data_map: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    expression_to_work_ids = context.get("expression_to_work_ids", {})
    ...
    for exp in raw_expressions:
        ...
        parent_work_ids = expression_to_work_ids.get(expression_id, [])
        if not parent_work_ids and (single_wid := expression_to_work.get(expression_id)):
            parent_work_ids = [single_wid]
            
        parent_work_id = parent_work_ids[0] if parent_work_ids else None
