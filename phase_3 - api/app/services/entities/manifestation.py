...
def process_manifestations(raw_manifestations: List[Dict[str, Any]], context: Dict[str, Any], works_data_map: Dict[str, Any], expressions_data_map: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Dict[str, Any], Dict[str, Any]]:
    ...
    volume_to_parent_manifestation_ids = context.get("volume_to_parent_manifestation_ids", {})
    manifestation_to_expression = context["manifestation_to_expression"]
    manifestation_to_expression_ids = context.get("manifestation_to_expression_ids", {})
    ...
    for man in raw_manifestations:
        ...
        if man_id in volume_ids:
            ...
            parent_manifestation_ids = volume_to_parent_manifestation_ids.get(man_id, [])
            if not parent_manifestation_ids and (single_pmid := volume_to_parent_manifestation_id.get(man_id)):
                parent_manifestation_ids = [single_pmid]
            parent_manifestation_id = parent_manifestation_ids[0] if parent_manifestation_ids else None

            parent_expression_ids = []
            for pm_id in parent_manifestation_ids:
                for e_id in manifestation_to_expression_ids.get(pm_id, [manifestation_to_expression.get(pm_id)] if manifestation_to_expression.get(pm_id) else []):
                    if e_id and e_id not in parent_expression_ids:
                        parent_expression_ids.append(e_id)
            parent_expression_id = parent_expression_ids[0] if parent_expression_ids else None

            work_ids = []
            authors = []
            projects = id2projects.get(man_id, ["Unknown Project"])

            for e_id in parent_expression_ids:
                if e_details := expressions_data_map.get(e_id):
                    for w_id in e_details.get("work_ids", [e_details.get("work_id")] if e_details.get("work_id") else []):
                        if w_id and w_id not in work_ids:
                            work_ids.append(w_id)
                    if not id2projects.get(man_id):
                        projects = e_details.get("projects", ["Unknown Project"])

            for w_id in work_ids:
                if w_id and works_data_map.get(w_id):
                    for a in works_data_map[w_id].get("authors", []):
                        if a not in authors:
                            authors.append(a)
            ...
            vol_data = {
                ...
                "parent_manifestation_ids": parent_manifestation_ids,
                "expression_id": parent_expression_id, 
                "expression_ids": parent_expression_ids,
                "work_id": work_id, 
                "work_ids": work_ids,
                "projects": projects, 
        ...
        parent_expression_ids = manifestation_to_expression_ids.get(man_id, [])
        if not parent_expression_ids and (single_eid := manifestation_to_expression.get(man_id)):
            parent_expression_ids = [single_eid]
        ...
        for p_eid in parent_expression_ids:
            if p_exp_details := expressions_data_map.get(p_eid):
                if expression_title == "Unknown Expression":
                    expression_title = p_exp_details.get("expression_title", expression_title)
                for w_id in p_exp_details.get("work_ids", [p_exp_details.get("work_id")] if p_exp_details.get("work_id") else []):
                    if w_id and w_id not in work_ids:
                        work_ids.append(w_id)
                if work_title == "Unknown Work":
                    work_title = p_exp_details.get("work_title", work_title)
                for a in p_exp_details.get("authors", []):
                    if a not in authors:
                        authors.append(a)
                author_search_terms_normalized.update(p_exp_details.get("author_search_terms_normalized", set()))
                for cl in p_exp_details.get("classifications", []):
                    if cl not in classifications: classifications.append(cl)
                classifications_normalized.update(p_exp_details.get("classifications_normalized", set()))
                for toe in p_exp_details.get("type_of_expression", []):
                    if toe not in type_of_expression: type_of_expression.append(toe)
                type_of_expression_normalized.update(p_exp_details.get("type_of_expression_normalized", set()))
                for lang in p_exp_details.get("language", []):
                    if lang not in language: language.append(lang)
                language_normalized.update(p_exp_details.get("language_normalized", set()))
                work_title_normalized.update(p_exp_details.get("work_title_normalized", set()))

                for tr in p_exp_details.get("translators", []):
                    if tr not in translators: translators.append(tr)
                for ee in p_exp_details.get("expression_editors", []):
                    if ee not in expression_editors: expression_editors.append(ee)
                for sw in p_exp_details.get("scriptwriters", []):
                    if sw not in scriptwriters: scriptwriters.append(sw)
                for comp in p_exp_details.get("compositors", []):
                    if comp not in compositors: compositors.append(comp)
                for rev in p_exp_details.get("reviewers", []):
                    if rev not in reviewers: reviewers.append(rev)
                for osr in p_exp_details.get("other_secondary_roles", []):
                    if osr not in other_secondary_roles: other_secondary_roles.append(osr)

                if not id2projects.get(man_id):
                    projects = p_exp_details.get("projects", ["Unknown Project"])
