...
def apply_all_ancestors(
    ...
    def inject_item_ancestors(cache, i_data):
        for w_id in i_data.get('work_ids', [i_data.get('work_id')] if i_data.get('work_id') else []):
            if w_id and (w_data := works_data_map.get(w_id)):
                inject_ancestor(cache, w_id, 'work', w_data['work_title'], w_data['card'])
        for e_id in i_data.get('expression_ids', [i_data.get('expression_id')] if i_data.get('expression_id') else []):
            if e_id and (e_data := expressions_data_map.get(e_id)):
                inject_ancestor(cache, e_id, 'expression', e_data['expression_title'], e_data['card'])
        for m_id in i_data.get('manifestation_ids', [i_data.get('manifestation_id')] if i_data.get('manifestation_id') else []):
            if m_id and (m_data := manifestations_data_map.get(m_id)):
                inject_ancestor(cache, m_id, 'manifestation', m_data['manifestation_title'], m_data['card'])

    def inject_volume_ancestors(cache, v_data):
        for w_id in v_data.get('work_ids', [v_data.get('work_id')] if v_data.get('work_id') else []):
            if w_id and (w_data := works_data_map.get(w_id)):
                inject_ancestor(cache, w_id, 'work', w_data['work_title'], w_data['card'])
        for e_id in v_data.get('expression_ids', [v_data.get('expression_id')] if v_data.get('expression_id') else []):
            if e_id and (e_data := expressions_data_map.get(e_id)):
                inject_ancestor(cache, e_id, 'expression', e_data['expression_title'], e_data['card'])
        for m_id in v_data.get('parent_manifestation_ids', [v_data.get('parent_manifestation_id')] if v_data.get('parent_manifestation_id') else []):
            if m_id and (m_data := manifestations_data_map.get(m_id)):
                inject_ancestor(cache, m_id, 'manifestation', m_data['manifestation_title'], m_data['card'])

    def inject_manifestation_ancestors(cache, m_data):
        for w_id in m_data.get('work_ids', [m_data.get('work_id')] if m_data.get('work_id') else []):
            if w_id and (w_data := works_data_map.get(w_id)):
                inject_ancestor(cache, w_id, 'work', w_data['work_title'], w_data['card'])
        for e_id in m_data.get('expression_ids', [m_data.get('expression_id')] if m_data.get('expression_id') else []):
            if e_id and (e_data := expressions_data_map.get(e_id)):
                inject_ancestor(cache, e_id, 'expression', e_data['expression_title'], e_data['card'])
    ...
    for exp in data_maps["expressions_list"]:
        ...
        for w_id in exp.get('work_ids', [exp.get('work_id')] if exp.get('work_id') else []):
            if w_id and (w_data := works_data_map.get(w_id)):
                inject_ancestor(cache, w_id, 'work', w_data['work_title'], w_data['card'])
    ...
    for man in data_maps["manifestations_list"]:
        ...
        for w_id in man.get('work_ids', [man.get('work_id')] if man.get('work_id') else []):
            if w_id and (w_data := works_data_map.get(w_id)):
                inject_ancestor(cache, w_id, 'work', w_data['work_title'], w_data['card'])
        for e_id in man.get('expression_ids', [man.get('expression_id')] if man.get('expression_id') else []):
            if e_id and (e_data := expressions_data_map.get(e_id)):
                inject_ancestor(cache, e_id, 'expression', e_data['expression_title'], e_data['card'])
    ...
    for vol in data_maps["manifestation_volumes_list"]:
        ...
        for w_id in vol.get('work_ids', [vol.get('work_id')] if vol.get('work_id') else []):
            if w_id and (w_data := works_data_map.get(w_id)):
                inject_ancestor(cache, w_id, 'work', w_data['work_title'], w_data['card'])
        for e_id in vol.get('expression_ids', [vol.get('expression_id')] if vol.get('expression_id') else []):
            if e_id and (e_data := expressions_data_map.get(e_id)):
                inject_ancestor(cache, e_id, 'expression', e_data['expression_title'], e_data['card'])
        for m_id in vol.get('parent_manifestation_ids', [vol.get('parent_manifestation_id')] if vol.get('parent_manifestation_id') else []):
            if m_id and (m_data := manifestations_data_map.get(m_id)):
                inject_ancestor(cache, m_id, 'manifestation', m_data['manifestation_title'], m_data['card'])
    ...
    for item in data_maps["items_list"]:
        ...
        for w_id in item.get('work_ids', [item.get('work_id')] if item.get('work_id') else []):
            if w_id and (w_data := works_data_map.get(w_id)):
                inject_ancestor(cache, w_id, 'work', w_data['work_title'], w_data['card'])
        for e_id in item.get('expression_ids', [item.get('expression_id')] if item.get('expression_id') else []):
            if e_id and (e_data := expressions_data_map.get(e_id)):
                inject_ancestor(cache, e_id, 'expression', e_data['expression_title'], e_data['card'])
        for m_id in item.get('manifestation_ids', [item.get('manifestation_id')] if item.get('manifestation_id') else []):
            if m_id and (m_data := manifestations_data_map.get(m_id)):
                inject_ancestor(cache, m_id, 'manifestation', m_data['manifestation_title'], m_data['card'])
    ...
    for po in data_maps["physical_objects_list"]:
        ...
        hr_id = po.get('human_readable_id') or ""
                        ...
                        if gp_data := physical_objects_data_map.get(grandparent_po_id):
                            ...
                            if item_id := po_to_item_map.get(grandparent_po_id):
                                if i_data := items_data_map.get(item_id):
                                    ...
                                    inject_item_ancestors(cache, i_data)
    ...
    page_to_manifestation_ids = context.get("page_to_manifestation_ids", {})
    page_to_volume_ids = context.get("page_to_volume_ids", {})
    
    for page in data_maps["pages_list"]:
        p_id = page['page_id']
        cache = page_details_cache.get(p_id)
        hr_id = page.get('human_readable_id') or ""
        ...
        # Case c/d) Page is part of a Manifestation or Volume
            ...
            v_ids = page_to_volume_ids.get(p_id, [page_to_volume_map.get(p_id)] if page_to_volume_map.get(p_id) else [])
            m_ids = page_to_manifestation_ids.get(p_id, [page_to_manifestation_map.get(p_id)] if page_to_manifestation_map.get(p_id) else [])

            if v_ids:
                for v_id in v_ids:
                    if v_id and (v_data := manifestation_volumes_data_map.get(v_id)):
                        inject_ancestor(cache, v_id, 'manifestation_volume', v_data['manifestation_volume_title'], v_data['card'])
                        inject_volume_ancestors(cache, v_data)
            elif m_ids:
                for m_id in m_ids:
                    if m_id and (m_data := manifestations_data_map.get(m_id)):
                        inject_ancestor(cache, m_id, 'manifestation', m_data['manifestation_title'], m_data['card'])
                        inject_manifestation_ancestors(cache, m_data)
