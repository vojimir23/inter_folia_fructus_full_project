...
from app.services.common import translate_date_notes, get_contributor_details
...
def process_relations(rel_all: List[Dict[str, Any]], rtypes: Dict[str, Any], id2label: Dict[str, str], id_to_collection: Dict[str, str]) -> Dict[str, Any]:
    ...
    list_keys = [
        ...
        "expression_to_work_ids", "manifestation_to_expression_ids", "volume_to_parent_manifestation_ids",
        "item_to_manifestation_ids", "page_to_manifestation_ids", "page_to_volume_ids",
    ...
    ctx["hypothesis_to_description"] = {}
    ctx["hypothesis_to_creator_name"] = {}
    ...
    for r in rel_all:
        ...
        # --- EXPRESSION RELATIONS ---
            ...
            if e2 not in ctx["expression_to_work_ids"][e1]: ctx["expression_to_work_ids"][e1].append(e2)
        ...
        # --- MANIFESTATION & VOLUME RELATIONS ---
            ...
            if e2 not in ctx["manifestation_to_expression_ids"][e1]: ctx["manifestation_to_expression_ids"][e1].append(e2)
            ctx["manifestation_to_expression"][e1] = e2
        elif rname == "manifestation_has_volume": 
            ctx["manifestation_to_volume_ids"][e1].append(e2)
            if e1 not in ctx["volume_to_parent_manifestation_ids"][e2]: ctx["volume_to_parent_manifestation_ids"][e2].append(e1)
        ...
        # --- ITEM RELATIONS ---
            ...
            if e2 not in ctx["item_to_manifestation_ids"][e1]: ctx["item_to_manifestation_ids"][e1].append(e2)
            ...
            # CRITICAL: Also link the item to the volume's PARENT manifestation(s)
            parent_manifestation_ids = ctx["volume_to_parent_manifestation_ids"].get(e2, [])
            for pm_id in parent_manifestation_ids:
                if pm_id not in ctx["item_to_manifestation_ids"][e1]: ctx["item_to_manifestation_ids"][e1].append(pm_id)
            if parent_manifestation_ids:
                ctx["item_to_manifestation"][e1] = parent_manifestation_ids[0]
        ...
        elif rname == "page_from_manifestation": 
            if e2 not in ctx["page_to_manifestation_ids"][e1]: ctx["page_to_manifestation_ids"][e1].append(e2)
            ctx["page_to_manifestation_map"][e1] = e2
        elif rname == "page_from_manifestation_volume": 
            if e2 not in ctx["page_to_volume_ids"][e1]: ctx["page_to_volume_ids"][e1].append(e2)
            ctx["page_to_manifestation_volume_map"][e1] = e2
        ...
        elif rname == "hypothesis_has_description":
            ctx["hypothesis_to_description"][e1] = id2label.get(e2, "")
    ...
    # Hydrate hypothesis creator, about, and description details
    for entity_id, hypos in ctx["entity_to_hypotheses_details"].items():
        for hypo in hypos:
            hypo_id = hypo["hypothesis_id"]

            # Description
            if hypo_id in ctx["hypothesis_to_description"]:
                hypo["hypothesis_description"] = ctx["hypothesis_to_description"][hypo_id]
            else:
                hypo["hypothesis_description"] = ""

            # Creator
            creator_id = ctx["hypothesis_to_creator_name"].get(hypo_id)
            if creator_id:
                creator_name = ctx["person_to_name"].get(creator_id, id2label.get(creator_id, "Unknown Creator"))
                hypo["creator_name"] = creator_name
                
                creator_details = get_contributor_details(creator_id, ctx)
                if creator_details:
                    hypo["creator"] = {k: v for k, v in creator_details.items() if v is not None}
                else:
                    coll_type = id_to_collection.get(creator_id, "person")
                    hypo["creator"] = {
                        "id": creator_id,
                        "name": creator_name,
                        "type": coll_type
            ...
            about_ids = ctx["hypothesis_to_about_ids"].get(hypo_id, [])
            about_list = []
            for about_id in about_ids:
                coll = id_to_collection.get(about_id, "unknown")
                label = id2label.get(about_id, "Unknown")
            ...
            hypo["hypothesis_about"] = about_list
