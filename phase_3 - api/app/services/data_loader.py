...
from app.services.common import extract_label, translate_date_notes
...
async def load_and_process_data(db: AsyncIOMotorDatabase) -> None:
    ...
    try:
        ...
        attribute_relations = {
            ...
            "page_has_material", "person_has_birth_date", "person_has_death_date", "person_has_birth_date_notes", "person_has_death_date_notes"
        }
        ...
        for r in rel_all:
            ...
            if rel_name in attribute_relations:
                if e1_cache := all_caches.get(e1):
                    target_label = id2label.get(e2, "")
                    if rel_name == "page_has_material":
                        target_label = PAGE_MATERIAL_MAP.get(target_label.lower(), target_label)
                    elif rel_name in ["person_has_birth_date", "person_has_death_date", "person_has_birth_date_notes", "person_has_death_date_notes"]:
                        target_label = translate_date_notes(target_label) or target_label

                    add_relationship(e1_cache, {"type": rel_name, "direction": "outgoing", "group": "other", "target_id": e2, "target_type": "literal", "target_label": target_label, "target_card": {}})
                continue
